...
const STORAGE_KEY = 'rp_plan_full_v14';
...
const DEFAULT_PLAN = {
  ...
  demographics: {
    ...
    statePensionPart: 6000,
    terminalAge: 100,
  },
  spending: {
    targetSpend: 25000,
    staggeredSpend: 15000,
    taperAge: 80,
    taperRate: 15.0,
  ...
  otherIncomes: [
    {
      id: 'inc_1',
      name: 'DB pension / Consulting',
      owner: 'Myself',
      startAge: 57,
      endAge: 100,
      amount: 0,
      taxTreatment: 'Tax-free',
      notes: 'Representative stream',
    },
  ],
  oneOffContributions: [
    {
      ...
      category: 'Pensions',
      amount: 40000,
      desc: 'One-Off Lump Sum Contribution',
    },
  ],
  oneOffCosts: [],
  config: {
...
function grossPensionNeededForNet(
  netTarget,
  otherTaxableIncome = 0,
  config,
  isFullyCrystallized = false,
  pclsHeadroom = 268275,
  maxTaxableCeiling = Infinity
) {
  ...
  for (let i = 0; i < 22; i++) {
    ...
    const newNetWith = calculateUKNetIncome(
      otherTaxableIncome + taxablePart,
      config
    );
...
export default function App() {
  ...
  const [plan, setPlan] = useState(() => {
    try {
      ...
      if (cached) {
        ...
        if (!parsed.spending.decumulationPolicy)
          parsed.spending.decumulationPolicy = 'Bracket Fill';
        return parsed;
      }
      return DEFAULT_PLAN;
    } catch (e) {
  ...
  // =========================================================================
  // 3. UNIFIED SIMULATION ENGINE (UK FIRE Bracket Fill Decumulation)
  // =========================================================================
  const runEngineYear = (
    t,
    potsMap,
    planState,
    regimeOrShock = 'expected',
    tracking = {
      cumPclsSelf: 0,
      cumPclsPart: 0,
      lumpSumTakenSelf: false,
      lumpSumTakenPart: false,
    }
  ) => {
    const ageSelfStart = Number(planState.demographics.currentAgeSelf) || 35;
    const agePartStart = Number(planState.demographics.currentAgePart) || 36;
    const retireAgeSelf = Number(planState.demographics.retireAgeSelf) || 57;
    const retireAgePart = Number(planState.demographics.retireAgePart) || 57;
    const privatePenAge =
      Number(planState.demographics.privatePensionAge) || 58;
    const statePenAge = Number(planState.demographics.statePensionAge) || 68;

    const taperFraction = (Number(planState.spending.taperRate) || 15) / 100;
    const taperAge = Number(planState.spending.taperAge) || 80;
    const targetSpend = Number(planState.spending.targetSpend) || 25000;

    const staggeredSpend =
      planState.spending.staggeredSpend !== '' &&
      planState.spending.staggeredSpend !== undefined
    ...
    const baseYear = planState.config.valuationDate
      ? parseInt(planState.config.valuationDate.slice(0, 4))
      : 2026;
    ...
    planState.oneOffContributions
      .filter((c) => {
        const itemYear = c.date
          ? parseInt(c.date.slice(0, 4))
          : Number(c.year) || year;
        return itemYear === year;
      })
      .forEach((c) => {
        const targetAcc = planState.accounts.find(
          (a) => a.owner === c.owner && a.category === c.category
        );
    ...
    planState.accounts.forEach((acc) => {
      ...
      if (isWorking && contrib > 0) {
        potsMap[acc.id] +=
          contrib * Math.pow(1 + growth / 100, t) * fractionThisYear;
      }
    });
    ...
    if (planState.spending.drawdownStrategy === 'Full 25% Lump Sum') {
      ...
      if (canAccessSelf && !tracking.lumpSumTakenSelf && potsMap.pen_self > 0) {
        const pcls = Math.min(
          potsMap.pen_self * pclsProp,
          Math.max(0, lsaCap - tracking.cumPclsSelf)
        );
      ...
      if (canAccessPart && !tracking.lumpSumTakenPart && potsMap.pen_part > 0) {
        const pcls = Math.min(
          potsMap.pen_part * pclsProp,
          Math.max(0, lsaCap - tracking.cumPclsPart)
        );
    ...
    // 4. One-off Capital Costs (Waterfall: Cash -> Other -> ISA -> Pension 58+)
    const costThisYear = planState.oneOffCosts
      .filter((c) => {
        const itemYear = c.date
          ? parseInt(c.date.slice(0, 4))
          : Number(c.year) || year;
    ...
    planState.otherIncomes.forEach((inc) => {
      ...
      const active =
        inc.owner === 'Myself'
          ? ageSelf >= sAge && ageSelf <= eAge
          : agePart >= sAge && agePart <= eAge;
    ...
    const spSelf =
      ageSelf >= statePenAge
        ? Number(planState.demographics.statePensionSelf) || 0
        : 0;
    const spPart =
      agePart >= statePenAge
        ? Number(planState.demographics.statePensionPart) || 0
        : 0;

    let currentTaxableSelf = otherTaxableSelf + spSelf;
    let currentTaxablePart = otherTaxablePart + spPart;

    const baseNetIncomeSelf =
      otherNetSelf + calculateUKNetIncome(currentTaxableSelf, planState.config);
    const baseNetIncomePart =
      otherNetPart + calculateUKNetIncome(currentTaxablePart, planState.config);
    ...
    if (!workingSelf && !workingPart) {
      annualLivingTarget =
        ageSelf >= taperAge ? targetSpend * (1 - taperFraction) : targetSpend;
    ...
    if (annualLivingTarget > 0 && totalNetGuaranteed >= annualLivingTarget) {
      ...
      const isFullLump =
        planState.spending.drawdownStrategy === 'Full 25% Lump Sum';

      const drawFromPension = (
        potOwner,
        netNeeded,
        maxTaxableCeiling = Infinity
      ) => {
        ...
        const pclsHeadroom = Math.max(
          0,
          lsaCap -
            (potOwner === 'Myself'
              ? tracking.cumPclsSelf
              : tracking.cumPclsPart)
        );
        const taxBase =
          potOwner === 'Myself' ? currentTaxableSelf : currentTaxablePart;

        if (taxBase >= maxTaxableCeiling) return 0;

        const grossNeeded = grossPensionNeededForNet(
          netNeeded,
          taxBase,
          planState.config,
          isFullLump,
        ...
        const newNet = calculateUKNetIncome(
          taxBase + taxableTaken,
          planState.config
        );
      ...
      if (ageSelf < privatePenAge) {
        ...
        if (demandSelf > 0 || demandPart > 0)
          drawTier('other_self', 'other_part');
        ...
        if (demandPart > 0 && currentTaxableSelf < paAllowance) {
          const crossCoverS = drawFromPension(
            'Myself',
            demandPart,
            paAllowance
          );
        ...
        if (demandSelf > 0 && currentTaxablePart < paAllowance) {
          const crossCoverP = drawFromPension(
            'Partner',
            demandSelf,
            paAllowance
          );
        ...
        if (demandSelf > 0 || demandPart > 0) {
          if (demandSelf > 0)
            demandSelf = Math.max(
              0,
              demandSelf - drawFromPension('Myself', demandSelf)
            );
          if (demandPart > 0)
            demandPart = Math.max(
              0,
              demandPart - drawFromPension('Partner', demandPart)
            );
          if (demandPart > 0)
            demandPart = Math.max(
              0,
              demandPart - drawFromPension('Myself', demandPart)
            );
          if (demandSelf > 0)
            demandSelf = Math.max(
              0,
              demandSelf - drawFromPension('Partner', demandSelf)
            );
        }
      } else {
        drawTier('cash_self', 'cash_part');
    ...
    planState.accounts.forEach((acc) => {
      const profile =
        planState.riskProfiles && planState.riskProfiles[acc.risk]
          ? planState.riskProfiles[acc.risk]
          : DEFAULT_RISK_PROFILES[acc.risk] ||
            DEFAULT_RISK_PROFILES['High Risk'];
      ...
      if (regimeOrShock === 'lucky') {
        ...
        const assetVol =
          acc.risk === 'Cash Equivalents'
            ? 0.005
            : Math.max(
                0.01,
                regimeOrShock.sigma * Math.max(0.1, realRate / 0.0444)
              );
        const assetDrift = realRate - 0.5 * assetVol * assetVol;
        growthRate = Math.exp(assetDrift + assetVol * z) - 1;
      }

      potsMap[acc.id] = Math.max(
        0,
        potsMap[acc.id] * (1 + growthRate * compoundFactor)
      );
    });
  ...
  const timelineData = useMemo(() => {
    ...
    for (let t = 0; t <= totalYears; t++) {
      ...
      const stepUnlucky = runEngineYear(
        t,
        potsUnlucky,
        plan,
        'unlucky',
  ...
  const auditMetrics = useMemo(() => {
    ...
    const retRow =
      timelineData.find(
        (r) => r.ageSelf === (Number(plan.demographics.retireAgeSelf) || 57)
      ) || timelineData[0];
    const postRetRows = timelineData.filter(
      (r) => r.ageSelf >= (Number(plan.demographics.retireAgeSelf) || 57)
    );
    const troughVal = postRetRows.length
      ? Math.min(...postRetRows.map((r) => r.totalCombined))
      : 0;
    const terminalVal =
      timelineData[timelineData.length - 1]?.totalCombined || 0;
    return {
      startVal,
      retAge: Number(plan.demographics.retireAgeSelf) || 57,
      retVal: retRow.totalCombined,
      troughVal,
      terminalVal,
    };
  ...
  const executeSimulation = (spendAmount, trials = 1000) => {
    ...
    const targetSpend = Number(plan.spending.targetSpend) || 25000;
    const staggeredSpend =
      plan.spending.staggeredSpend !== '' &&
      plan.spending.staggeredSpend !== undefined
    ...
    const minRetireAge = Math.min(
      Number(plan.demographics.retireAgeSelf) || 57,
      Number(plan.demographics.retireAgePart) || 57
    );
    ...
    for (let i = 0; i < trials; i++) {
      const trialPots = {};
      plan.accounts.forEach((acc) => {
        trialPots[acc.id] = Number(acc.balance) || 0;
      });
      const trialTracking = {
        cumPclsSelf: 0,
        cumPclsPart: 0,
  ...
  const handleRunMC = () => {
    ...
    setTimeout(() => {
      const spend = Number(plan.spending.targetSpend) || 25000;
      const res = executeSimulation(spend, 1000);
      setSimResult({
        type: 'test',
        title: 'Current Baseline Plan Stress Test',
        spend,
        ...res,
      });
  ...
  const handleOptimize = () => {
    ...
    setTimeout(() => {
      ...
      let low = Math.max(10000, Math.floor((base * 0.4) / 1000) * 1000);
      ...
      while (high - low > 250) {
        ...
        if (res.successRate >= targetConfidence) {
          opt = mid;
          low = mid;
        } else high = mid;
      }
  ...
  const maxY = useMemo(() => {
    ...
    return Math.max(max * 1.08, 500000);
  }, [visibleData, activeSeries]);

  const yScale = useMemo(() => {
    return d3.scaleLinear().domain([0, maxY]).range([innerHeight, 0]).nice();
  }, [maxY, innerHeight]);
    ...
    setPlan((prev) => ({
      ...
      accounts: prev.accounts.map((a) =>
        a.id === id
          ? {
              ...a,
              [field]: field === 'risk' ? value : parseInputNumber(value),
            }
          : a
      ),
    }));
  ...
  const updateDemographics = (field, value) => {
    setPlan((prev) => ({
      ...prev,
      demographics: { ...prev.demographics, [field]: parseInputNumber(value) },
    }));
  };
  ...
  const updateSpending = (field, value) => {
    setPlan((prev) => ({
      ...
      spending: {
        ...prev.spending,
        [field]:
          field === 'drawdownStrategy' || field === 'decumulationPolicy'
            ? value
  ...
  const updateConfig = (field, value) => {
    setPlan((prev) => ({
      ...
      config: {
        ...prev.config,
        [field]: field === 'valuationDate' ? value : parseInputNumber(value),
      },
    }));
  };
  ...
  const addOneOffContrib = () => {
    const newC = {
      ...
      category: 'Pensions',
      amount: 10000,
      desc: 'One-off Injection',
    };
    setPlan((prev) => ({
      ...prev,
      oneOffContributions: [...prev.oneOffContributions, newC],
    }));
  };
  ...
  const deleteOneOffContrib = (id) => {
    setPlan((prev) => ({
      ...prev,
      oneOffContributions: prev.oneOffContributions.filter((c) => c.id !== id),
    }));
  };
  ...
  const addOneOffCost = () => {
    const newCost = {
      ...
      owner: 'Myself',
      amount: 15000,
      desc: 'Home Renovation / Wedding',
    };
    setPlan((prev) => ({
      ...prev,
      oneOffCosts: [...prev.oneOffCosts, newCost],
    }));
  };
  ...
  const deleteOneOffCost = (id) => {
    setPlan((prev) => ({
      ...prev,
      oneOffCosts: prev.oneOffCosts.filter((c) => c.id !== id),
    }));
  };
  ...
  const handleExportJSON = () => {
    ...
    downloadAnchor.setAttribute(
      'download',
      `retirement_plan_${new Date().toISOString().slice(0, 10)}.json`
    );
  ...
  const handleResetDefaults = () => {
    if (
      confirm(
        'Reset all inputs, return matrices, and portfolio accounts back to baseline defaults?'
      )
    ) {
        ...
        <div className="bg-white border border-slate-200/90 rounded-2xl p-5 shadow-xs flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div>
            <div className="flex items-center gap-2.5">
              ...
              <span className="text-[11px] font-mono px-2 py-0.5 rounded-md bg-blue-50 text-blue-700 font-semibold border border-blue-100">
                v3.2
              </span>
            </div>
            <p className="text-xs text-slate-500 mt-1">
              UK multi-wrapper drawdown model and Monte Carlo stress tester
            </p>
          </div>
        ...
        {activeTab === 'inputs' && (
          <div className="space-y-6">
            <div className="flex flex-wrap items-center justify-between gap-3 bg-white border border-slate-200/90 p-4 rounded-2xl shadow-xs">
              <div>
                <h2 className="text-sm font-bold text-slate-900 uppercase tracking-wider">
                  User Inputs & Wrapper Portfolios
                </h2>
                <p className="text-xs text-slate-500">
                  ...
                  to move between fields. Numbers highlight on focus so you can
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  ...
                  className="px-3 py-1.5 bg-rose-50 hover:bg-rose-100 border border-rose-200 text-rose-700 rounded-xl text-xs font-semibold flex items-center gap-1.5 transition-all cursor-pointer"
                >
            ...
            <div className="bg-white border border-slate-200/90 p-5 rounded-2xl shadow-xs space-y-4">
              <div>
                <h3 className="text-xs font-bold text-blue-700 uppercase tracking-wider flex items-center gap-2">
                  <Users className="w-4 h-4 text-blue-600" /> 1. Demographics &
                  Retirement Targets
                </h3>
                <div className="mt-2 p-3 bg-blue-50/60 border border-blue-100 rounded-xl text-xs text-slate-600 flex items-start gap-2.5">
                  <Info className="w-4 h-4 text-blue-600 shrink-0 mt-0.5" />
                  <span>
                    <strong>Decumulation Note:</strong> With{' '}
                    <em>Bracket Fill</em> selected, the model takes just enough
                    pension from age 58 to use up your 0% Personal Allowance,
                    then draws ISAs to cover the rest without triggering 20% or
                    40% income tax.
                  </span>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 text-xs">
                <div>
                  ...
                  <input
                    ...
                    placeholder="0"
                    onFocus={handleFocus}
                    value={plan.demographics.currentAgeSelf}
                    onChange={(e) =>
                      updateDemographics('currentAgeSelf', e.target.value)
                    }
                ...
                <div>
                  ...
                  <input
                    ...
                    placeholder="0"
                    onFocus={handleFocus}
                    value={plan.demographics.currentAgePart}
                    onChange={(e) =>
                      updateDemographics('currentAgePart', e.target.value)
                    }
                    className="w-full p-2 bg-slate-50 border border-slate-300 rounded-lg font-mono text-slate-900 font-bold focus:bg-white focus:ring-2 focus:ring-blue-500 focus:outline-none"
                  />
                </div>
                <div>
                  <label className="text-slate-600 font-semibold block mb-1">
                    Retirement Age (Myself)
                  </label>
                  <input
                    type="number"
                    placeholder="0"
                    onFocus={handleFocus}
                    value={plan.demographics.retireAgeSelf}
                    onChange={(e) =>
                      updateDemographics('retireAgeSelf', e.target.value)
                    }
                    className="w-full p-2 bg-slate-50 border border-slate-300 rounded-lg font-mono text-slate-900 font-bold focus:bg-white focus:ring-2 focus:ring-blue-500 focus:outline-none"
                  />
                </div>
                <div>
                  ...
                  <input
                    ...
                    placeholder="0"
                    onFocus={handleFocus}
                    value={plan.demographics.retireAgePart}
                    onChange={(e) =>
                      updateDemographics('retireAgePart', e.target.value)
                    }
                    className="w-full p-2 bg-slate-50 border border-slate-300 rounded-lg font-mono text-slate-900 font-bold focus:bg-white focus:ring-2 focus:ring-blue-500 focus:outline-none"
                  />
                </div>
                <div>
                  <label className="text-slate-600 font-semibold block mb-1">
                    Expected State Pension Myself (£/yr)
                  </label>
                  <input
                    type="number"
                    step="250"
                    placeholder="0"
                    onFocus={handleFocus}
                    value={plan.demographics.statePensionSelf}
                    onChange={(e) =>
                      updateDemographics('statePensionSelf', e.target.value)
                    }
                    className="w-full p-2 bg-slate-50 border border-slate-300 rounded-lg font-mono text-slate-900 font-bold focus:bg-white focus:ring-2 focus:ring-blue-500 focus:outline-none"
                  />
                </div>
                <div>
                  <label className="text-slate-600 font-semibold block mb-1">
                    Expected State Pension Partner (£/yr)
                  </label>
                  <input
                    type="number"
                    step="250"
                    placeholder="0"
                    onFocus={handleFocus}
                    value={plan.demographics.statePensionPart}
                    onChange={(e) =>
                      updateDemographics('statePensionPart', e.target.value)
                    }
                    className="w-full p-2 bg-slate-50 border border-slate-300 rounded-lg font-mono text-slate-900 font-bold focus:bg-white focus:ring-2 focus:ring-blue-500 focus:outline-none"
                  />
                </div>
                <div>
                  ...
                  <input
                    ...
                    placeholder="0"
                    onFocus={handleFocus}
                    value={plan.spending.targetSpend}
                    onChange={(e) =>
                      updateSpending('targetSpend', e.target.value)
                    }
                    className="w-full p-2 bg-slate-50 border border-slate-300 rounded-lg font-mono text-slate-900 font-bold focus:bg-white focus:ring-2 focus:ring-blue-500 focus:outline-none"
                  />
                </div>
                <div>
                  ...
                  <input
                    ...
                    placeholder="0"
                    onFocus={handleFocus}
                    value={plan.spending.staggeredSpend}
                    onChange={(e) =>
                      updateSpending('staggeredSpend', e.target.value)
                    }
                    className="w-full p-2 bg-slate-50 border border-slate-300 rounded-lg font-mono text-slate-900 font-bold focus:bg-white focus:ring-2 focus:ring-blue-500 focus:outline-none"
                  />
                </div>
                <div>
                  ...
                  <input
                    ...
                    placeholder="0"
                    onFocus={handleFocus}
                    value={plan.spending.taperAge}
                    onChange={(e) => updateSpending('taperAge', e.target.value)}
                    className="w-full p-2 bg-slate-50 border border-slate-300 rounded-lg font-mono text-slate-900 font-bold focus:bg-white focus:ring-2 focus:ring-blue-500 focus:outline-none"
                  />
                </div>
                <div>
                  <label className="text-slate-600 font-semibold block mb-1">
                    Spend Taper Reduction (%)
                  </label>
                  <div className="relative">
                    <input
                      ...
                      placeholder="0"
                      onFocus={handleFocus}
                      value={plan.spending.taperRate}
                      onChange={(e) =>
                        updateSpending('taperRate', e.target.value)
                      }
                      className="w-full p-2 bg-slate-50 border border-slate-300 rounded-lg font-mono text-slate-900 font-bold pr-8 focus:bg-white focus:ring-2 focus:ring-blue-500 focus:outline-none"
                    />
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      updateSpending('decumulationPolicy', e.target.value)
                    }
                  ...
                  >
                    <option value="Bracket Fill">
                      UK FIRE Bracket Fill (Tax-Optimized)
                    </option>
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      updateSpending('drawdownStrategy', e.target.value)
                    }
                    className="w-full p-2 bg-slate-50 border border-slate-300 rounded-lg text-xs text-blue-700 font-bold focus:bg-white focus:ring-2 focus:ring-blue-500 focus:outline-none cursor-pointer"
                  >
                    <option value="Phased Drawdown">
                      Phased Drawdown (Ongoing 25% tax-free proportion)
                    </option>
                    <option value="Full 25% Lump Sum">
                      Full 25% Lump Sum (Upfront statutory PCLS into Cash)
                    </option>
                  </select>
                </div>
              </div>
            ...
            <div className="bg-white border border-slate-200/90 p-5 rounded-2xl shadow-xs space-y-4 overflow-x-auto">
              <div>
                <h3 className="text-xs font-bold text-blue-700 uppercase tracking-wider flex items-center gap-2">
                  <Wallet className="w-4 h-4 text-blue-600" /> 2. Current
                  Balances, Annual Savings & Risk Profiles
                </h3>
                <div className="mt-2 p-3 bg-blue-50/60 border border-blue-100 rounded-xl text-xs text-slate-600 space-y-1.5">
                  <div className="flex items-start gap-2.5">
                    <Info className="w-4 h-4 text-blue-600 shrink-0 mt-0.5" />
                    <span>
                      Enter starting balances and current annual savings. Each
                      pot compounds at the real rate assigned to its risk level.
                    </span>
                  </div>
                  <div className="flex items-start gap-2.5 pt-1 text-amber-800 font-medium border-t border-blue-100/60">
                    <AlertTriangle className="w-4 h-4 text-amber-600 shrink-0 mt-0.5" />
                    <span>
                      <strong>Tax Note:</strong> This version does not deduct
                      CGT or dividend taxes from 'Other Investments' (GIAs) due
                      to individual cost basis variations. If you rely heavily
                      on taxable accounts, your net results will be somewhat
                      optimistic.
                    </span>
                  </div>
                </div>
                    ...
                    >
                      ...
                      <td className="py-2.5">
                        <input
                          ...
                          value={acc.growth}
                          onChange={(e) =>
                            updateAccountField(acc.id, 'growth', e.target.value)
                          }
                      ...
                      <td className="py-2.5">
                        <select
                          value={acc.risk}
                          onChange={(e) =>
                            updateAccountField(acc.id, 'risk', e.target.value)
                          }
            ...
            <div className="bg-white border border-slate-200/90 p-5 rounded-2xl shadow-xs space-y-4">
              <div className="flex justify-between items-center">
                <div>
                  <h3 className="text-xs font-bold text-blue-700 uppercase tracking-wider flex items-center gap-2">
                    <Coins className="w-4 h-4 text-blue-600" /> 3. Expected
                    Other Income Streams (DB Pension, Consulting, Rental)
                  </h3>
                  <span className="text-[11px] text-slate-500">
                    Taxable streams count toward personal allowance and tax
                    bands; tax-free streams directly reduce net drawdown demand.
                  </span>
                </div>
                <button
                  onClick={addOtherIncome}
                  className="px-2.5 py-1 bg-blue-600 hover:bg-blue-700 text-white rounded-lg text-xs font-bold flex items-center gap-1 cursor-pointer shadow-xs"
                >
              ...
              {plan.otherIncomes.length === 0 ? (
                <div className="text-xs text-slate-400 italic p-3 bg-slate-50 border border-slate-200 rounded-xl">
                  No additional income streams registered.
                </div>
              ) : (
                <div className="space-y-2">
                  {plan.otherIncomes.map((inc) => (
                    <div
                      key={inc.id}
                      className="grid grid-cols-1 sm:grid-cols-6 gap-2 p-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs items-center"
                    >
                      <input
                        type="text"
                        onFocus={handleFocus}
                        ...
                        onChange={(e) =>
                          setPlan((p) => ({
                            ...p,
                            otherIncomes: p.otherIncomes.map((i) =>
                              i.id === inc.id
                            ...
                            setPlan((p) => ({
                              ...
                              otherIncomes: p.otherIncomes.map((i) =>
                                i.id === inc.id
                                  ? {
                                      ...i,
                                      endAge: parseInputNumber(e.target.value),
                                    }
                                  : i
                              ),
                            }))
                            ...
                            setPlan((p) => ({
                              ...
                              otherIncomes: p.otherIncomes.map((i) =>
                                i.id === inc.id
                                  ? {
                                      ...i,
                                      amount: parseInputNumber(e.target.value),
                                    }
                                  : i
                              ),
                            }))
                      ...
                      <div className="flex justify-end">
                        <button
                          onClick={() => deleteOtherIncome(inc.id)}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="bg-white border border-slate-200/90 p-5 rounded-2xl shadow-xs space-y-4">
                <div className="flex justify-between items-center">
                  <div>
                    <h3 className="text-xs font-bold text-blue-700 uppercase tracking-wider flex items-center gap-2">
                      <Plus className="w-4 h-4 text-blue-600" /> 4. One-Off
                      Injections (by Wrapper)
                    </h3>
                    <span className="text-[11px] text-slate-500">
                      Injects capital into a chosen account in a specific
                      calendar year.
                    </span>
                  </div>
                  <button
                    onClick={addOneOffContrib}
                    className="px-2.5 py-1 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-lg text-xs font-bold flex items-center gap-1 cursor-pointer border border-slate-200"
                  >
                ...
                {plan.oneOffContributions.length === 0 ? (
                  <div className="text-xs text-slate-400 italic p-3 bg-slate-50 border border-slate-200 rounded-xl">
                    No one-off contributions scheduled.
                  </div>
                ) : (
                  <div className="space-y-2">
                    {plan.oneOffContributions.map((c) => (
                      <div
                        key={c.id}
                        className="flex flex-wrap items-center gap-2 p-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs"
                      >
                        <input
                          type="date"
                          ...
                          onChange={(e) => {
                            ...
                            setPlan((p) => ({
                              ...p,
                              oneOffContributions: p.oneOffContributions.map(
                                (x) =>
                                  x.id === c.id
                                    ? { ...x, date: d, year: yr }
                                    : x
                        ...
                        >
                          ...
                          <option value="Other Investments">
                            Other Investments
                          </option>
                        ...
                        />
                        <button
                          onClick={() => deleteOneOffContrib(c.id)}
                          className="p-1 ml-auto text-slate-400 hover:text-rose-600 cursor-pointer transition-colors"
                        >
                          <Trash2 className="w-4 h-4" />
                        </button>
                      </div>
              ...
              <div className="bg-white border border-slate-200/90 p-5 rounded-2xl shadow-xs space-y-4">
                <div className="flex justify-between items-center">
                  <div>
                    <h3 className="text-xs font-bold text-rose-700 uppercase tracking-wider flex items-center gap-2">
                      <Trash2 className="w-4 h-4 text-rose-600" /> 5. One-Off
                      Capital Costs (Waterfall)
                    </h3>
                    <span className="text-[11px] text-slate-500">
                      Pulls sequentially: Cash → Other (GIA) → ISA → Pension
                      (58+).
                    </span>
                  </div>
                  <button
                ...
                {plan.oneOffCosts.length === 0 ? (
                  <div className="text-xs text-slate-400 italic p-3 bg-slate-50 border border-slate-200 rounded-xl">
                    No one-off capital expenses scheduled.
                  </div>
                ) : (
                  <div className="space-y-2">
                      ...
                      >
                        <input
                          ...
                          onChange={(e) => {
                            ...
                            setPlan((p) => ({
                              ...p,
                              oneOffCosts: p.oneOffCosts.map((x) =>
                                x.id === cost.id
                                  ? { ...x, date: d, year: yr }
                                  : x
                              ),
                        ...
                        />
                        <button
                          onClick={() => deleteOneOffCost(cost.id)}
                          className="p-1 text-slate-400 hover:text-rose-600 cursor-pointer transition-colors"
                        >
                          <Trash2 className="w-4 h-4" />
                        </button>
          ...
          <div className="space-y-6">
            <div className="bg-white border border-slate-200/90 p-5 rounded-2xl shadow-xs space-y-4">
              <h2 className="text-sm font-bold text-slate-900 uppercase tracking-wider flex items-center gap-2">
                <Settings className="w-4 h-4 text-blue-600" /> Global Economic &
                Calculation Configuration
              </h2>
              <p className="text-xs text-slate-500">
                Economic and regulatory tax settings used throughout the
                projection engine.
              </p>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 text-xs pt-3">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      updateConfig('valuationDate', e.target.value)
                    }
                    className="w-full p-2 bg-slate-50 border border-slate-300 rounded-lg font-mono text-slate-900 font-bold focus:bg-white focus:ring-2 focus:ring-blue-500 focus:outline-none"
                  />
                  <span className="text-[10px] text-slate-400 mt-1 block">
                    Year fraction remaining is calculated automatically.
                  </span>
                </div>
                <div>
                  <label className="text-slate-600 font-semibold block mb-1">
                    Headline Inflation CPI (% pa)
                  </label>
                  <input
                    type="number"
                    step="0.1"
                    placeholder="0"
                    onFocus={handleFocus}
                    value={plan.config.inflation}
                    onChange={(e) => updateConfig('inflation', e.target.value)}
                    className="w-full p-2 bg-slate-50 border border-slate-300 rounded-lg font-mono text-slate-900 font-bold focus:bg-white focus:ring-2 focus:ring-blue-500 focus:outline-none"
                  />
                </div>
                <div>
                  <label className="text-slate-600 font-semibold block mb-1">
                    Personal Pension Access Age (NMPA)
                  </label>
                  <input
                    type="number"
                    placeholder="0"
                    onFocus={handleFocus}
                    value={plan.demographics.privatePensionAge}
                    onChange={(e) =>
                      updateDemographics('privatePensionAge', e.target.value)
                    }
                    className="w-full p-2 bg-slate-50 border border-slate-300 rounded-lg font-mono text-slate-900 font-bold focus:bg-white focus:ring-2 focus:ring-blue-500 focus:outline-none"
                  />
                </div>
                <div>
                  ...
                  <input
                    ...
                    placeholder="0"
                    onFocus={handleFocus}
                    value={plan.demographics.statePensionAge}
                    onChange={(e) =>
                      updateDemographics('statePensionAge', e.target.value)
                    }
                    className="w-full p-2 bg-slate-50 border border-slate-300 rounded-lg font-mono text-slate-900 font-bold focus:bg-white focus:ring-2 focus:ring-blue-500 focus:outline-none"
                  />
                </div>
                <div>
                  ...
                  <input
                    ...
                    placeholder="0"
                    onFocus={handleFocus}
                    value={plan.config.annualVolatility}
                    onChange={(e) =>
                      updateConfig('annualVolatility', e.target.value)
                    }
                    className="w-full p-2 bg-slate-50 border border-slate-300 rounded-lg font-mono text-slate-900 font-bold focus:bg-white focus:ring-2 focus:ring-blue-500 focus:outline-none"
                  />
                </div>
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      updateConfig('solvencyFloor', e.target.value)
                    }
                  ...
                  />
                  <span className="text-[10px] text-slate-400 mt-1 block">
                    Minimum required capital at age 100 to pass a trial.
                  </span>
                </div>
              </div>
            </div>
              ...
              <div className="flex justify-between items-center">
                <div>
                  <h3 className="text-xs font-bold text-blue-700 uppercase tracking-wider">
                    Investment Risk Profiles & Real Return Matrix
                  </h3>
                  <span className="text-[11px] text-slate-500">
                    Real annual returns net of fees. Click the pencil icon to
                    edit assumptions.
                  </span>
                </div>
                <button
                      ...
                      <td className="py-2.5">
                        {isEditingRisk ? (
                          <input
                            ...
                            value={val.real}
                            onChange={(e) =>
                              updateRiskField(key, 'real', e.target.value)
                            }
                      ...
                      <td className="py-2.5">
                        {isEditingRisk ? (
                          <input
                            ...
                            value={val.unlucky}
                            onChange={(e) =>
                              updateRiskField(key, 'unlucky', e.target.value)
                            }
                      ...
                      <td className="py-2.5">
                        {isEditingRisk ? (
                          <input
                            ...
                            value={val.lucky}
                            onChange={(e) =>
                              updateRiskField(key, 'lucky', e.target.value)
                            }
                      ...
                      <td className="py-2.5">
                        {isEditingRisk ? (
                          <input
                            ...
                            value={val.nominal}
                            onChange={(e) =>
                              updateRiskField(key, 'nominal', e.target.value)
                            }
            ...
            <div className="bg-white border border-slate-200/90 p-5 rounded-2xl shadow-xs space-y-4">
              <h3 className="text-xs font-bold text-blue-700 uppercase tracking-wider">
                UK Income Tax Bands & Pension Allowances
              </h3>
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 text-xs font-mono">
                <div>
                  <span className="text-slate-600 font-sans font-semibold block mb-1">
                    Personal Allowance (£)
                  </span>
                  <input
                    type="number"
                    placeholder="0"
                    onFocus={handleFocus}
                    value={plan.config.personalAllowance}
                    onChange={(e) =>
                      updateConfig('personalAllowance', e.target.value)
                    }
                    className="w-full p-2 bg-slate-50 border border-slate-300 rounded font-bold text-slate-900 focus:bg-white focus:ring-2 focus:ring-blue-500 focus:outline-none"
                  />
                </div>
                <div>
                  <span className="text-slate-600 font-sans font-semibold block mb-1">
                  ...
                  <input
                    ...
                    placeholder="0"
                    onFocus={handleFocus}
                    value={plan.config.paTaperThreshold}
                    onChange={(e) =>
                      updateConfig('paTaperThreshold', e.target.value)
                    }
                    className="w-full p-2 bg-slate-50 border border-slate-300 rounded font-bold text-slate-900 focus:bg-white focus:ring-2 focus:ring-blue-500 focus:outline-none"
                  />
                </div>
                <div>
                  ...
                  <input
                    ...
                    placeholder="0"
                    onFocus={handleFocus}
                    value={plan.config.basicBandLimit}
                    onChange={(e) =>
                      updateConfig('basicBandLimit', e.target.value)
                    }
                    className="w-full p-2 bg-slate-50 border border-slate-300 rounded font-bold text-slate-900 focus:bg-white focus:ring-2 focus:ring-blue-500 focus:outline-none"
                  />
                </div>
                <div>
                  <span className="text-slate-600 font-sans font-semibold block mb-1">
                    Higher Rate Band Limit (£)
                  </span>
                  <input
                    type="number"
                    placeholder="0"
                    onFocus={handleFocus}
                    value={plan.config.higherBandLimit}
                    onChange={(e) =>
                      updateConfig('higherBandLimit', e.target.value)
                    }
                    className="w-full p-2 bg-slate-50 border border-slate-300 rounded font-bold text-slate-900 focus:bg-white focus:ring-2 focus:ring-blue-500 focus:outline-none"
                  />
                </div>
                <div>
                  ...
                  <input
                    ...
                    placeholder="0"
                    onFocus={handleFocus}
                    value={plan.config.pclsProportion}
                    onChange={(e) =>
                      updateConfig('pclsProportion', e.target.value)
                    }
                    className="w-full p-2 bg-slate-50 border border-slate-300 rounded font-bold text-slate-900 focus:bg-white focus:ring-2 focus:ring-blue-500 focus:outline-none"
                  />
                </div>
                <div>
                  <span className="text-slate-600 font-sans font-semibold block mb-1">
                    PCLS Statutory Cap (£ LSA)
                  </span>
                  <input
                    type="number"
                    placeholder="0"
                    onFocus={handleFocus}
                    value={plan.config.pclsMaxCap}
                    onChange={(e) => updateConfig('pclsMaxCap', e.target.value)}
                    className="w-full p-2 bg-slate-50 border border-slate-300 rounded font-bold text-slate-900 focus:bg-white focus:ring-2 focus:ring-blue-500 focus:outline-none"
                  />
                </div>
              </div>
          ...
          <div className="space-y-6">
            <div className="p-3.5 bg-blue-50/70 border border-blue-200/80 rounded-2xl text-xs text-slate-700 flex items-start gap-3">
              ...
              <div>
                ...
                <span>
                  <strong>Test Current Spend</strong> tests your target annual
                  spend against 1,000 randomized market return paths.{' '}
                  <strong>Safe Max Annual Spend</strong> solves for the highest
                  budget your portfolio can sustain to age 100 at your chosen
            ...
            <div className="bg-white border border-slate-200/90 rounded-2xl p-4 shadow-xs flex flex-wrap items-center justify-between gap-3">
              <div className="flex items-center gap-2">
                ...
                {['Combined', 'Myself', 'Partner'].map((p) => (
                  <button
                    key={p}
                    onClick={() =>
                      setPlan((prev) => ({ ...prev, activeProfileView: p }))
                    }
                    className={`px-3 py-1 rounded-xl text-xs font-bold transition-all cursor-pointer ${
                      plan.activeProfileView === p
              ...
              <div className="flex items-center gap-2">
                ...
                >
                  ...
                  {isOptimizing
                    ? 'Solving...'
                    : `⚡ Safe Max Annual Spend (${targetConfidence}%)`}
                </button>
              </div>
            </div>
            ...
            {simResult && (
              <div
                className={`p-5 rounded-2xl shadow-xs border transition-all ${
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="bg-white border border-slate-200/90 p-5 rounded-2xl shadow-xs">
                <div className="text-xs font-semibold uppercase tracking-wider text-slate-500">
                  Expected Terminal Pot ({plan.activeProfileView})
                </div>
                <div className="text-2xl font-black font-mono text-blue-600 mt-2">
                  {formatGBP(
                    chartDisplayData[chartDisplayData.length - 1]?.expected
                  )}
                </div>
                <div className="text-xs text-slate-500 mt-1 flex items-center gap-1.5">
                  <Target className="w-3.5 h-3.5 text-blue-600" /> Projected
                  balance at age 100
                </div>
              </div>
              <div className="bg-white border border-slate-200/90 p-5 rounded-2xl shadow-xs">
                <div className="text-xs font-semibold uppercase tracking-wider text-slate-500">
                  Lucky Scenario (90th %ile)
                </div>
            ...
            <div className="bg-white border border-slate-200/90 p-5 rounded-2xl shadow-xs space-y-4">
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
                <div>
                  <h2 className="text-sm font-bold text-slate-900 uppercase tracking-wider flex items-center gap-2">
                    <Layers className="w-4 h-4 text-blue-600" /> Projected
                    Portfolio Trajectory ({plan.activeProfileView})
                  </h2>
                ...
                <div className="flex items-center gap-3 bg-slate-50 border border-slate-200 px-3 py-1.5 rounded-xl text-xs w-full sm:w-auto">
                  <span className="text-slate-600 whitespace-nowrap">
                    Horizon: <strong>Age {maxVisibleAge}</strong>
                  </span>
                  <input
                    type="range"
                    min="50"
                  ...
                  <g transform={`translate(${margin.left}, ${margin.top})`}>
                    ...
                    {showMilestones && (
                      <>
                        ...
                        {(Number(plan.demographics.privatePensionAge) || 58) <=
                          maxVisibleAge && (
                          <g
                            transform={`translate(${xScale(
                              Number(plan.demographics.privatePensionAge) || 58
                          ...
                          >
                            ...
                            >
                              NMPA (
                              {Number(plan.demographics.privatePensionAge) ||
                                58}
                              )
                            </text>
                          </g>
                        )}
                        {(Number(plan.demographics.statePensionAge) || 68) <=
                          maxVisibleAge && (
                          <g
                            transform={`translate(${xScale(
                              Number(plan.demographics.statePensionAge) || 68
                    ...
                    {SERIES_CONFIG.map((s) => {
                      if (!activeSeries[s.id] || !pathGenerators[s.id])
                        return null;
                      return (
                        <path
                          key={s.id}
                          d={pathGenerators[s.id]}
                          fill="none"
                          stroke={s.color}
                    ...
                    <rect
                      ...
                      onMouseMove={(e) => {
                        ...
                        const age = Math.round(
                          xScale.invert(e.clientX - rect.left)
                        );
                        const point = visibleData.find(
                          (d) => d.ageSelf === age
                        );
                    ...
                    {hoveredPoint && (
                      ...
                      >
                        ...
                        />
                        <circle
                          cy={yScale(hoveredPoint.expected)}
                          r="4"
                          fill="#2563eb"
                          stroke="#ffffff"
                          strokeWidth="2"
                ...
                {hoveredPoint && (
                  <div className="absolute top-4 left-24 bg-white/95 border border-slate-200 p-3 rounded-xl shadow-lg text-xs space-y-1 backdrop-blur-md pointer-events-none">
                    <div className="font-bold text-slate-800 border-b border-slate-100 pb-1 flex justify-between gap-4">
                      <span>
                        Age {hoveredPoint.ageSelf} ({hoveredPoint.year})
                      </span>
                      <span className="text-slate-500">
                        Spend Demand: {formatGBP(hoveredPoint.targetSpend)}/yr
                      </span>
                    </div>
                    <div className="grid grid-cols-2 gap-x-4 gap-y-1 pt-1 font-mono">
                      {activeSeries.expected && (
                        <div className="text-blue-600 font-bold">
                          {plan.activeProfileView}:{' '}
                          {formatGBP(hoveredPoint.expected)}
                        </div>
                      )}
                      {activeSeries.lucky && (
                        <div className="text-emerald-600">
                          Lucky: {formatGBP(hoveredPoint.lucky)}
                        </div>
                      )}
                      {activeSeries.unlucky && (
                        <div className="text-rose-600">
                          Unlucky: {formatGBP(hoveredPoint.unlucky)}
                        </div>
                      )}
                      {activeSeries.pensions && (
                        <div className="text-sky-600">
                          Pensions: {formatGBP(hoveredPoint.pensions)}
                        </div>
                      )}
                      {activeSeries.isas && (
                        <div className="text-teal-600">
                          ISAs: {formatGBP(hoveredPoint.isas)}
                        </div>
                      )}
                    </div>
                  </div>
            ...
            {auditMetrics && (
              <div className="bg-white border border-slate-200/90 p-5 rounded-2xl shadow-xs space-y-3">
                <h3 className="text-sm font-bold text-slate-900 uppercase tracking-wider flex items-center gap-2">
                  <ShieldCheck className="w-4 h-4 text-emerald-600" /> Plan
                  Benchmarks
                </h3>
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-xs font-mono">
                  <div className="bg-slate-50 border border-slate-200 p-3 rounded-xl">
                    ...
                    <span className="text-base font-bold text-slate-900">
                      {formatGBP(auditMetrics.startVal)}
                    </span>
                  </div>
                  <div className="bg-slate-50 border border-slate-200 p-3 rounded-xl">
                    <span className="text-slate-500 font-sans font-semibold block mb-1">
                      Pot at Retirement (Age {auditMetrics.retAge})
                    </span>
                    <span className="text-base font-bold text-amber-700">
                      {formatGBP(auditMetrics.retVal)}
                    </span>
                  </div>
                  <div className="bg-slate-50 border border-slate-200 p-3 rounded-xl">
                    <span className="text-slate-500 font-sans font-semibold block mb-1">
                    ...
                    <span className="text-base font-bold text-emerald-700">
                      {formatGBP(auditMetrics.troughVal)}
                    </span>
                  </div>
                  <div className="bg-slate-50 border border-slate-200 p-3 rounded-xl">
                    <span className="text-slate-500 font-sans font-semibold block mb-1">
                    ...
                    <span className="text-base font-bold text-blue-700">
                      {formatGBP(auditMetrics.terminalVal)}
                    </span>
                  </div>
                </div>
              </div>
            ...
            <div className="flex justify-between items-center flex-wrap gap-2">
              <div>
                <h2 className="text-sm font-bold text-slate-900 uppercase tracking-wider flex items-center gap-2">
                  ...
                  Year-by-Year Cash Flow Audit
                </h2>
                <p className="text-xs text-slate-500">
                  Granular annual ledger matching the underlying spreadsheet
                  columns.
                </p>
              </div>
              <button
              ...
              <table className="w-full text-left text-xs border-collapse font-mono whitespace-nowrap">
                <thead className="sticky top-0 bg-slate-100 border-b border-slate-200 text-slate-600 z-10 font-bold">
                  <tr>
                    ...
                    <th className="p-2.5 text-blue-700 font-bold">
                      Combined (Real)
                    </th>
                    <th className="p-2.5 text-purple-700">
                      Combined (Nominal)
                    </th>
                    ...
                    <th className="p-2.5 text-emerald-700">
                      Pre-58 Liquid Equity
                    </th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-100">
                  {timelineData.map((r, idx) => (
                    ...
                    >
                      <td className="p-2.5 font-bold text-slate-900 sticky left-0 bg-inherit border-r border-slate-100">
                        {r.year}
                      </td>
                      ...
                      <td className="p-2.5 text-amber-700 font-bold">
                        {formatGBP(r.netDrawdown)}
                      </td>
                      <td className="p-2.5 font-bold text-blue-700 bg-blue-50/30">
                        {formatGBP(r.totalCombined)}
                      </td>
                      <td className="p-2.5 text-purple-700">
                        {formatGBP(r.nominal)}
                      </td>
                      ...
                      <td className="p-2.5 text-sky-700">
                        {formatGBP(r.pensions)}
                      </td>
                      ...
                      <td className="p-2.5 text-rose-700">
                        {formatGBP(r.drawdownPensions)}
                      </td>
                      <td className="p-2.5 text-emerald-800 font-semibold">
                        {formatGBP(r.pre58LiquidEquity)}
                      </td>
                    </tr>
                  ))}
                </tbody>
            ...
            <div className="bg-white border border-slate-200/90 rounded-2xl p-5 shadow-xs lg:sticky lg:top-6 space-y-3">
              ...
              <nav className="space-y-1 text-xs">
                {[
                  {
                    id: 'doc-philosophy',
                    label: '1. Architecture & Real Terms',
                  },
                  {
                    id: 'doc-timeline',
                    label: '2. Timeline & Mid-Year Starts',
                  },
                  { id: 'doc-incomes', label: '3. Guaranteed Income & UK Tax' },
                  { id: 'doc-gia-tax', label: '4. Note on GIA / Other Tax' },
                  {
                    id: 'doc-surplus',
                    label: '5. What Happens to Surplus Income',
                  },
                  {
                    id: 'doc-decumulation',
                    label: '6. Bracket Fill vs Sequential Drawdown',
                  },
                  {
                    id: 'doc-pension-rules',
                    label: '7. Phased Drawdown vs 25% Lump Sum',
                  },
                  {
                    id: 'doc-spousal',
                    label: '8. Spousal Absorption & Tax Tracking',
                  },
                  { id: 'doc-one-offs', label: '9. How One-Offs Are Treated' },
                  {
                    id: 'doc-monte-carlo',
                    label: '10. Monte Carlo & Safe Max Spend',
                  },
                ].map((item) => (
                  <button
                    key={item.id}
            ...
            <div className="lg:col-span-3 space-y-8 bg-white border border-slate-200/90 rounded-2xl p-6 sm:p-8 shadow-xs text-slate-700 text-sm leading-relaxed">
              <div className="pb-6 border-b border-slate-100">
                <h2 className="text-xl font-bold text-slate-900">
                  Model Logic & Reference Guide
                </h2>
                <p className="text-slate-500 text-xs mt-1">
                  How the model handles UK tax bands, wrapper liquidation order,
              ...
              <section id="doc-philosophy" className="space-y-3 pt-2">
                ...
                <p>
                  The model runs entirely in <strong>real terms</strong>{' '}
                  (today's purchasing power) rather than nominal pounds. If you
                  enter £25,000/yr, that represents £25,000 of goods and
                  services whether you are 35, 65, or 90. Asset return rates are
                  net of inflation.
                </p>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-1">
                  <div className="p-3.5 bg-slate-50 border border-slate-200/80 rounded-xl text-xs space-y-1">
                    ...
                    <p className="text-slate-600">
                      If an equity fund returns 7.05% nominal and inflation is
                      2.5%, its real return is 4.44%. Living spend targets
                      remain constant instead of compounding by inflation each
                      year.
                    </p>
                  </div>
                  <div className="p-3.5 bg-slate-50 border border-slate-200/80 rounded-xl text-xs space-y-1">
                    <strong className="text-slate-900 block font-semibold">
                      Four Separate Account Wrappers
                    </strong>
                    <p className="text-slate-600">
                      Wealth is divided across four accounts per person (eight
                      total): Pensions, S&S ISAs, Other Investments (GIA), and
                      Cash. Each wrapper has its own access age and tax rules.
                    </p>
                  </div>
                </div>
              </section>
              ...
              >
                ...
                <p>
                  The model projects annually from the year of your valuation
                  date to age 100.
                </p>
                <p>
                  Because life rarely starts on January 1st, Year 0 ($t=0$) is
                  prorated:
                </p>
                <div className="p-3.5 bg-blue-50/50 border border-blue-100 rounded-xl text-xs text-slate-700 space-y-1 font-mono">
                  <div>
                    Year Fraction Remaining = (End of Year - Valuation Date) /
                    365 Days
                  </div>
                  <div className="text-slate-500 font-sans mt-1">
                    For example, starting in September leaves roughly 31.5% of
                    the year. In Year 0, contributions and annual growth are
                    scaled by 0.315 so balances reflect where you will actually
                    be at year-end.
                  </div>
                </div>
              </section>
              ...
              >
                ...
                <p>
                  Before liquidating any invested assets, the model totals your
                  non-portfolio income:
                </p>
                <ul className="list-disc pl-5 space-y-1 text-xs text-slate-600">
                  <li>
                    <strong>State Pension:</strong> Unlocks at your chosen state
                    pension age (age 68 by default). Treated as taxable income.
                  </li>
                  <li>
                    <strong>Tax-Free Income:</strong> Certain DB lump sums or
                    allowances bypass the tax engine and reduce net spending
                    needs pound-for-pound.
                  </li>
                  <li>
                    <strong>Taxable Income:</strong> DB pensions, annuities, and
                    consulting income combine with your State Pension.
                  </li>
                </ul>
                <p>Taxable income is routed through standard UK tax bands:</p>
                <div className="p-3.5 bg-slate-50 border border-slate-200/80 rounded-xl text-xs space-y-2">
                  ...
                  <div className="flex justify-between">
                    <span>£0 to £12,570 (Personal Allowance)</span>
                    <span className="font-bold text-emerald-700">0%</span>
                  </div>
                  <div className="flex justify-between">
                    <span>£12,570 to £50,270 (Basic Rate)</span>
                    <span className="font-bold text-blue-700">20%</span>
                  </div>
                  <div className="flex justify-between">
                    <span>£50,270 to £125,140 (Higher Rate)</span>
                    <span className="font-bold text-amber-700">40%</span>
                  </div>
                  <div className="flex justify-between">
                    <span>Over £125,140 (Additional Rate)</span>
                    <span className="font-bold text-rose-700">45%</span>
                  </div>
                  <div className="text-[11px] text-slate-500 pt-1 border-t border-slate-200">
                    The £100k taper is included: £1 of Personal Allowance is
                    removed for every £2 of income above £100,000.
                  </div>
                </div>
              </section>
              ...
              >
                ...
                <div className="p-4 bg-amber-50/80 border border-amber-200 rounded-xl text-xs text-amber-900 space-y-2">
                  ...
                  <p>
                    In this version, withdrawals from{' '}
                    <strong>Other Investments (GIAs)</strong> are treated as
                    gross = net. We don't model Capital Gains Tax (CGT) or
                    dividend tax because they depend heavily on individual
                    circumstances:
                  </p>
                  <ul className="list-disc pl-5 space-y-1 text-slate-700">
                    <li>
                      Your historical purchase price and Section 104 cost
                      pooling.
                    </li>
                    <li>
                      How much growth comes from share price increases vs
                      dividend payouts.
                    </li>
                    <li>
                      Whether you harvest gains annually inside the £3,000 CGT
                      exemption.
                    </li>
                  ...
                  <p className="pt-1 font-semibold text-amber-950 border-t border-amber-200/80">
                    What this means: If a significant portion of your retirement
                    spending comes from taxable brokerage accounts rather than
                    pensions and ISAs, your projected portfolio balance will be
              ...
              >
                ...
                <p>
                  If guaranteed income (like State Pensions or DB payouts)
                  exceeds your living spend in a given year, portfolio
                  withdrawals drop to £0.
                </p>
                <p>
                  The surplus cash does not vanish. It is split 50/50 and
                  deposited into each partner's{' '}
                  <strong>Tier 1 Cash Savings</strong>, where it earns the cash
                  return rate and stands ready to fund future spending.
                </p>
              </section>
                ...
                <div className="space-y-3 pt-1">
                  <div className="p-4 bg-emerald-50/50 border border-emerald-200 rounded-xl space-y-2 text-xs">
                    ...
                    <p className="text-slate-700">
                      Standard sequential drawdown burns ISAs down to £0 before
                      touching pensions. That wastes your £12,570 tax-free
                      personal allowance during early retirement. Bracket Fill
                      fixes this:
                    </p>
                    <ol className="list-decimal pl-5 space-y-1 text-slate-700">
                      <li>
                        <strong>Before 58:</strong> Spends Cash $\rightarrow$
                        GIA $\rightarrow$ ISAs to bridge the gap. Pensions are
                        locked.
                      </li>
                      <li>
                        <strong>Age 58+ (Personal Allowance):</strong> Draws
                        pension money first to fill your remaining 0% Personal
                        Allowance (£12,570 taxable, or ~£16.7k gross under
                        phased drawdown). This money comes out tax-free.
                      </li>
                      <li>
                        <strong>Age 58+ (Taxable Accounts):</strong> Drains GIA
                        to cut down on tax drag.
                      </li>
                      <li>
                        <strong>Age 58+ (ISAs as a Tax Shield):</strong> Taps
                        ISAs next. This keeps your taxable income at £0 and
                        prevents you from creeping into the 20% or 40% income
                        tax bands.
                      </li>
                      <li>
                        <strong>Age 58+ (Basic Rate Band):</strong> If ISAs run
                        dry, draws pension money up to the £50,270 Basic Rate
                        limit.
                      </li>
                      <li>
                        <strong>Age 58+ (Higher Rate):</strong> Any remaining
                        spending need draws across higher brackets.
                      </li>
                    </ol>
                  </div>
                  ...
                  <div className="p-4 bg-slate-50 border border-slate-200 rounded-xl space-y-2 text-xs">
                    ...
                    <p className="text-slate-600">
                      Drains accounts in rigid order:{' '}
                      <code>Cash → Other (GIA) → ISAs → Pensions (58+)</code>.
                      This burns through every penny of ISAs before taking a
                      single pound from pensions. It mirrors older spreadsheet
                      models.
                    </p>
                  </div>
                </div>
              ...
              >
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs">
                  <div className="p-3.5 bg-slate-50 border border-slate-200/80 rounded-xl space-y-1">
                    <strong className="text-slate-900 block font-semibold">
                      Phased Drawdown (Default)
                    </strong>
                    <p className="text-slate-600">
                      Each withdrawal is split:{' '}
                      <strong>25% tax-free cash (PCLS)</strong> and{' '}
                      <strong>75% taxable income</strong>. The rest of the pot
                      stays invested in the pension wrapper. The model solves
                      for the exact gross amount needed to meet your target
                      after tax.
                    </p>
                  </div>
                  <div className="p-3.5 bg-slate-50 border border-slate-200/80 rounded-xl space-y-1">
                    ...
                    <p className="text-slate-600">
                      At retirement (or age 58), 25% of the total pension is
                      taken immediately and moved into Cash Savings. Subsequent
                      pension withdrawals are 100% taxable as regular income.
                    </p>
                  </div>
                </div>
                <div className="p-3 bg-amber-50/60 border border-amber-200 rounded-xl text-xs text-amber-900">
                  <strong>The £268,275 Lump Sum Allowance (LSA) Cap:</strong> In
                  both options, the model tracks total tax-free cash taken. Once
                  an individual hits the statutory £268,275 cap, further pension
                  withdrawals become 100% taxable.
                </div>
              </section>
              ...
              >
                ...
                <p>
                  Partners rarely have identical pension balances. If one
                  partner has £17k and the other has £66k, drawing equally
                  without cross-funding would cause the smaller pot to run out
                  in a couple of years.
                </p>
                <div className="p-3.5 bg-slate-50 border border-slate-200/80 rounded-xl text-xs space-y-1.5">
                  <strong className="text-slate-900 block font-semibold">
                    How Cross-Absorption Works:
                  </strong>
                  <p className="text-slate-600">
                    1. Net living spend is split 50/50 between both partners.
                    <br />
                    2. Each partner funds their share from their own accounts at
                    the current tier.
                    <br />
                    3. If one partner's pot empties, the other partner's account
                    at that same tier covers the difference before moving to the
                    next wrapper.
                    <br />
                    4. <strong>Tax Bracket Tracking:</strong> When one partner
                    takes extra pension money to cover the other, the engine
                    tracks their taxable income so the additional withdrawal is
                    taxed at their true marginal rate rather than reusing lower
                    tax bands.
                  </p>
                </div>
              </section>
              ...
              >
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs">
                  <div className="p-3 bg-slate-50 border border-slate-200/80 rounded-xl space-y-1">
                    <strong className="text-slate-900 block font-semibold">
                      One-Off Injections (Section 4)
                    </strong>
                    <p className="text-slate-600">
                      Adds money to a chosen account in a specific calendar
                      year. The money begins compounding immediately at that
                      wrapper's assigned return rate.
                    </p>
                  </div>
                  <div className="p-3 bg-slate-50 border border-slate-200/80 rounded-xl space-y-1">
                    <strong className="text-slate-900 block font-semibold">
                      One-Off Costs (Section 5)
                    </strong>
                    <p className="text-slate-600">
                      Pulls sequentially:{' '}
                      <code>Cash → Other (GIA) → ISAs → Pensions (58+)</code>.
                      Using cash and ISAs first for large lump sums prevents a
                      temporary spike into the 40% income tax band.
                    </p>
                  </div>
                </div>
              </section>
              ...
              >
                ...
                <p>
                  Deterministic projections assume investments grow at a
                  constant rate every year. In reality, market timing matters:
                  poor returns in early retirement can deplete a portfolio even
                  if long-term averages look fine.
                </p>
                <div className="p-3.5 bg-slate-50 border border-slate-200/80 rounded-xl text-xs space-y-2">
                  <strong className="text-slate-900 block font-semibold">
                    The 1,000-Trial Model:
                  </strong>
                  <p className="text-slate-600">
                    The engine runs 1,000 randomized 65-year retirement paths
                    using Geometric Brownian Motion. Returns for each pot vary
                    based on the volatility of its risk tier.
                  </p>
                  <p className="text-slate-600">
                    <strong>Failure Condition:</strong> A trial fails if your
                    portfolio drops to or below your configured solvency floor
                    (£0 by default) or cannot fund living expenses after
                    retirement.
                  </p>
                  <p className="text-slate-600">
                    <strong>Safe Max Annual Spend:</strong> The solver runs a
                    binary search between £10k and £150k, finding the highest
                    annual net spending budget that survives to age 100 at your
                    chosen confidence level (e.g. 90% of trials).
                  </p>
                </div>
              </section>
            </div>
